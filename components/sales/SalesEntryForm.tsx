...
export function SalesEntryForm({ refreshData }: SalesEntryFormProps) {
  ...
  // Calculate total sale value whenever sale items change
  useEffect(() => {
    if (!isNoSale) {
  ...
  // Calculate total value for an individual sale item when carat or price per carat changes
  const updateSaleItemTotal = (index: number, field: keyof SaleItem, value: string) => {
    ...
    // Calculate total value if both carat and pricePerCarat exist
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // For regular sales, validate required fields for each sale item
      if (!isNoSale) {
