...
export function PasswordGate({ children, title, description }: PasswordGateProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/finance/verify-password', {
        method: 'POST',
        headers: {
      ...
      <Card className="w-full max-w-md bg-white">
        ...
        <CardContent>
          ...
          <p className="text-xs text-gray-500 text-center mt-4">
            This section requires special authorization to access financial data.
          </p>
        </CardContent>
      </Card>
    </div>
