...
import { cn, formatCurrency, generateMemoNumber } from "@/lib/utils";
...
// Lot B Memo schema - similar to regular memo but with more detailed stone fields
...
const lotBMemoSchema = z.object({
  ...
  memoTerms: z.number().min(1, "Memo terms must be at least 1 day"),
  items: z.array(lotBStoneSchema).min(1, "At least one item is required"),
});

type LotBMemoFormValues = z.infer<typeof lotBMemoSchema>;
...
export default function LotBMemoPage() {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [lastMemoNo, setLastMemoNo] = useState<string | null>(null);
  const [memoNoLoading, setMemoNoLoading] = useState(true);
  ...
  const defaultValues: LotBMemoFormValues = {
    memoNo: "",
    date: new Date(),
  ...
  const form = useForm<LotBMemoFormValues>({
    resolver: zodResolver(lotBMemoSchema),
    defaultValues,
  });
  ...
  // Fetch memo number
  useEffect(() => {
    const fetchMemoNumber = async () => {
      setMemoNoLoading(true);
      try {
        const response = await fetch("/api/memos/latest-number");
  ...
  const handleGenerateMemoNumber = () => {
    const newNumber = generateMemoNumber(lastMemoNo, form.getValues("date"));
    form.setValue("memoNo", newNumber);
    toast.success("Memo number generated");
  };
  ...
  const onSubmit = async (data: LotBMemoFormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || "Failed to create Lot B memo");
      }

      toast.success("Lot B Memo created successfully!");
      form.reset();
      setSelectedShipmentId("");
      
      // Redirect to memos list or show success
      window.location.href = "/memos";
    } catch (error) {
      console.error("Error creating Lot B memo:", error);
      toast.error(error instanceof Error ? error.message : "Failed to create Lot B memo");
    } finally {
  ...
  const getBackHref = () => {
    if (userRole === 'admin') return '/memos';
    if (userRole === 'employee') return '/memos';
    return '/';
  };
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">You don't have permission to create Lot B memos.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100">Create Lot B Memo</h1>
        </div>
      </div>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input
                  id="memoNo"
                  {...form.register("memoNo")}
                  placeholder="Enter memo number"
                />
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleGenerateMemoNumber}
                  disabled={memoNoLoading}
                >
                  {memoNoLoading ? (
            ...
            <div className="space-y-2">
              <Label htmlFor="memoTerms">Memo Terms (Days)</Label>
              <Input
                id="memoTerms"
                type="number"
                min="1"
                {...form.register("memoTerms", { 
                  valueAsNumber: true,
                  setValueAs: v => Number(v) || 30
                })}
              />
              {form.formState.errors.memoTerms && (
                <p className="text-sm text-red-500">{form.formState.errors.memoTerms.message}</p>
              )}
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              {!selectedShipmentId && (
                <p className="text-xs text-muted-foreground">Select a company before creating the Lot B memo.</p>
              )}
            </div>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  id="description"
                  placeholder="Optional notes about the memo"
